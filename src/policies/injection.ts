...
const PATTERNS: InjectionPattern[] = [
  // ── Shell ──
  {
    pattern: re("\\b(?:curl|wget)\\s+.*\\|\\s*(?:bash|sh|zsh)"),
    category: "shell_injection",
    description: "pipe to shell",
    sensitivities: ["low"],
  },
  {
    pattern: re("\\b(?:rm\\s+-rf\\b|\\bdel\\s+/F\\b)"),
    category: "shell_injection",
    description: "destructive ops",
    sensitivities: ["low"],
  },
  {
    pattern: re("[;|&$()`]\\s*(?:bash|sh|zsh|python|node|ruby)"),
    category: "shell_injection",
    description: "cmd chaining",
    sensitivities: ["medium"],
  },
  {
    pattern: re("\\b(?:nc|netcat|ncat)\\s+-[lL]"),
    category: "shell_injection",
    description: "reverse shell",
    sensitivities: ["medium"],
  },
  {
    pattern: re("\\b(?:/etc/(?:passwd|shadow)|C:\\\\Windows\\\\System32)\\b"),
    category: "shell_injection",
  ...
  {
    pattern: re("\\b(?:DROP\\s+TABLE|ALTER\\s+TABLE|TRUNCATE\\s+TABLE)\\b"),
    category: "sql_injection",
    description: "DDL",
    sensitivities: ["low"],
  },
  {
    pattern: re("\\b(?:UNION\\s+(?:ALL\\s+)?SELECT)\\b"),
    category: "sql_injection",
  ...
  {
    pattern: re(
      "\\b(?:ignore|forget|disregard)\\s+(?:all\\s+)?(?:previous|above|prior)\\s+(?:instructions?|directives?|commands?|context)\\b",
    ),
  ...
  {
    pattern: re("\\b(?:you\\s+are\\s+now|you\\s+must|you\\s+will)\\b"),
    category: "prompt_injection",
    description: "role hijack",
    sensitivities: ["medium"],
  },
  {
    pattern: re("\\b(?:system\\s+prompt|hidden\\s+instruction|secret\\s+rule)\\b"),
    category: "prompt_injection",
    description: "prompt probing",
    sensitivities: ["medium"],
  },
  {
    pattern: re("\\b(?:DAN\\b|jailbreak\\b|bypass.*filter)\\b"),
    category: "prompt_injection",
  ...
  {
    pattern: re("%2e%2e%2[fF]"),
    category: "path_traversal",
    description: "URL-encoded ../",
    sensitivities: ["medium"],
  },
  {
    pattern: re("/proc/(?:self|stat|cpuinfo|meminfo)"),
    category: "path_traversal",
    description: "Linux /proc",
    sensitivities: ["high"],
  },
];
