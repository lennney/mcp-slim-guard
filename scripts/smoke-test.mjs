...
async function test() {
  ...
  const mockServerPath = path.resolve(__dirname, "../dist/mock-server.js").replaceAll("\\", "/");
  fs.writeFileSync(
  ...
  try {
    ...
    console.log("\n--- tools/list ---");
    {
      ...
      console.log(`   Tools: ${listedToolNames.join(", ")}`);
      const ok =
        listedToolNames.includes("mock_echo") &&
        listedToolNames.includes("mock_add") &&
        listedToolNames.includes("mock_get_time") &&
        !listedToolNames.includes("mock_delete_something");
    ...
    console.log("\n--- tools/call mock_echo ---");
    {
      ...
      if (resp.error) {
        ...
        if (text.includes("hello-guard")) {
          console.log("   ✅ Echo passes through");
          passed++;
        } else {
    ...
    console.log("\n--- tools/call mock_add ---");
    {
      ...
      if (resp.error) {
        ...
        console.log(`   Result: ${text}`);
        if (text === "10") {
          console.log("   ✅ Add passes through");
          passed++;
        } else {
          console.log(`   ❌ Expected 10, got "${text}"`);
          failed++;
        }
      }
    }
    ...
    console.log("\n--- tools/call mock_delete_something (denied) ---");
    {
      ...
      if (resp.error || resp.result?.isError) {
        ...
        const hiddenFromDiscovery = !listedToolNames.includes("mock_delete_something");
        const rejectedCall =
          msg.includes("Unknown tool") ||
          msg.includes("deny") ||
          msg.includes("not allowed") ||
          msg.includes("blocked");
        if (hiddenFromDiscovery && rejectedCall) {
          console.log("   ✅ Denied tool hidden from discovery and rejected");
          passed++;
        } else {
          console.log("   ⚠️ Blocked but unexpected message");
          failed++;
        }
      } else {
    ...
    console.log("\n--- tools/call mock_get_time (x3) ---");
    {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (resp.error) console.log(`   Call ${i + 1}: ❌ ${resp.error.message}`);
          ...
          console.log(`   Call ${i + 1}: ${resp.result?.content?.[0]?.text || ""}`);
          ok++;
        }
      }
      if (ok === 3) {
        console.log("   ✅ All 3 passed (rate limit OK)");
        passed++;
      } else {
        console.log(`   ❌ ${3 - ok} failed`);
        failed++;
      }
    }
