...
title: mcp-slim-guard — MCP 压缩与安全代理
timestamp: "2026-07-23T12:00:00+08:00"
description: The control plane between AI agents and MCP servers — schema compression (83%+ token savings) + SSRF + injection detection + allow/deny + rate limiting + audit
tags:
  - mcp-slim-guard
  - agentinstruction
  - security
...
# mcp-slim-guard — MCP 压缩与安全代理

一句话：一行命令给 MCP 加上 Schema 压缩（83%+ token 节省）+ SSRF 防护 + 白名单 + 审计 + 限速 + 注入检测
...
npm test                    # 全量测试（402 tests, 20 files）
npx vitest run              # 同上
npx tsc --noEmit            # 类型检查
...
```
AI Agent → mcp-slim-guard (STDIO 或 HTTP 代理)
...
- 无损压缩：`mcp-slim-guard start --compressor light|tight`

## 协议标准

mcp-slim-guard 完全遵循 [MCP 标准协议](https://spec.modelcontextprotocol.io)：
...
```
mcp-slim-guard ──(MCP SDK)──→ 上游 MCP Server（GitHub / Playwright / ...）
```
...
| Phase 1 压缩对标     | ✅ 完成 | 5 级压缩 + lazy loading + 请求缓存 + CLI schema 统计，402 tests |
| 品牌改名             | ✅ 完成 | micro-mcp → mcp-slim-guard，npm 元数据国际化                    |
| 第 1 周发布准备      | 🔄 进行 | GitHub Release + demo GIF + Glama 更新（见 docs/ROADMAP.md）    |
| 第 2 周集中发布      | ⏳      | MCP Registry + Smithery + Awesome MCP Servers PR + 技术文章     |
...
- 架构设计: `docs/architecture-mcp-slim-guard.md`
...
- 2026-07-23 11:00: **Schema 统计嵌入 CLI** — 零依赖，真实字符数 + 透明 token 估算。`validate`/`init`/`start`/`status`/`doctor` 五个命令都展示了压缩对比（full/slim/wrapper 工具分类 + 字符数 + est. tokens）。
- 2026-07-23 10:00: **品牌改名** — micro-mcp → mcp-slim-guard。全仓改名 + package.json 元数据国际化 + Node.js 版本统一 + mcpName + server.json。
